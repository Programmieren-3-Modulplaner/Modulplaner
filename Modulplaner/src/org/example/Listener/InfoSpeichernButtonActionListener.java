...
public class InfoSpeichernButtonActionListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (modulNameTextfeld.getText().length() == 0) {
            JOptionPane.showMessageDialog(parent, "Bitte zuerst Namen eingeben!", "", JOptionPane.INFORMATION_MESSAGE, icon);
        } else if (profNameTextfeld.getText().length() == 0) {
            JOptionPane.showMessageDialog(parent, "Bitte zuerst Professor eingeben!", "", JOptionPane.INFORMATION_MESSAGE, icon);
            ...
            JOptionPane.showMessageDialog(parent, "Modul bereits vorhanden", "", JOptionPane.INFORMATION_MESSAGE, icon);
        } else if (note < 0 || (note > 0 && note < 1) || (note > 4 && note < 5) || note > 5) {
            JOptionPane.showMessageDialog(parent, "Zugelassene Noten: 0.0 (nicht bestanden) / 1.0 - 4.0 / 5.0", "", JOptionPane.INFORMATION_MESSAGE, icon);
        } else {
